#Region "Class frmViewEditStudentRegistration..."
Public Class frmViewEditStudentRegistration
#Region "Methods..."
    Private Sub LoadDataGrid()
        Try
            Dim i As Integer = 0
            dgviewregistration.Rows.Clear()
            cmd.CommandText = "select stuid,name,course,prehouseno,preaddress,precity,prepost,predist,prestate,prepin,precountry,homephone from StudentRegistration where InsID='" & My.Settings.LastOpenedInstituteID & "' and InsName='" & My.Settings.LastOpenedInstitute & "' and Period='" & My.Settings.Period & "' and SystemDate<='" & UmbrellaSystemDate & "'"
            ...
            While dr.Read()
                ...
                dgviewregistration.Rows(i).Cells(3).Value = dr("prehouseno").ToString() + ", " + dr("preaddress").ToString() + ", " + dr("precity").ToString() + ", " + dr("prepost").ToString() + ", " + dr("predist").ToString() + ", " + dr("prestate").ToString() + ", " + dr("prepin").ToString() + ", " + dr("precountry").ToString()
                dgviewregistration.Rows(i).Cells(4).Value = dr("homephone").ToString()
                i = i + 1
            End While
            dr.Close()
    ...
    Private Sub TotalRegistration()
        Try
            Dim row As Integer = dgviewregistration.Rows.Count
            EGroupBox1.GroupTitle = "Student List: " + row.ToString()
        Catch ex As Exception
    ...
    Private Sub OpenRecords()
        Try
            ...
            Dim k As Integer = dgviewregistration.CurrentRow.Index
            StuID = dgviewregistration.Rows(k).Cells(0).Value.ToString()
...
#End Region
#Region "Form Events..."
    Private Sub frmViewEditStudentRegistration_Load(ByVal sender As Object, ByVal e As System.EventArgs) Handles Me.Load
        LoadDataGrid()
        TotalRegistration()
    End Sub
#End Region
#Region "Button Events..."
    Private Sub btnexportexcel_Click(ByVal sender As Object, ByVal e As System.EventArgs) Handles btnexportexcel.ClickButtonArea
        Try
            If ((dgviewregistration.Columns.Count = 0) Or (dgviewregistration.Rows.Count = 0)) Then
            ...
            Else
                ...
                dset.Tables.Add()
                For i As Integer = 0 To dgviewregistration.ColumnCount - 1
                    dset.Tables(0).Columns.Add(dgviewregistration.Columns(i).HeaderText)
                Next
                Dim dr1 As DataRow
                For i As Integer = 0 To dgviewregistration.RowCount - 1
                    dr1 = dset.Tables(0).NewRow
                    For j As Integer = 0 To dgviewregistration.Columns.Count - 1
                        dr1(j) = dgviewregistration.Rows(i).Cells(j).Value
                    Next
        ...
        Try
            If txtsearch.Text <> "" Then
                ...
                ElseIf cmbsearch.Text = "Phone" Then
                    ...
                    dgviewregistration.ClearSelection()
                    For i = 0 To dgviewregistration.Rows.Count - 1
                        If dgviewregistration.Rows(i).Cells(4).Value.ToString().ToUpper() Like UCase(txtsearch.Text) Then
                            dgviewregistration.Rows(i).Selected = True
                            dgviewregistration.CurrentCell = dgviewregistration.Rows(i).Cells(0)
                            Exit For
                        End If
...
#End Region
#Region "DGViewEvents..."
    Private Sub dgviewregistration_CellClick(ByVal sender As Object, ByVal e As System.Windows.Forms.DataGridViewCellEventArgs) Handles dgviewregistration.CellClick
        Try
            Dim k As Integer = dgviewregistration.CurrentRow.Index
            dgviewregistration.Rows(k).Selected = True
            dgviewregistration.CurrentCell = dgviewregistration.Rows(k).Cells(0)
    ...
    Private Sub dgviewregistration_DoubleClick(ByVal sender As Object, ByVal e As System.EventArgs) Handles dgviewregistration.DoubleClick
        Try
            Dim k As Integer = dgviewregistration.CurrentRow.Index
            dgviewregistration.Rows(k).Selected = True
            dgviewregistration.CurrentCell = dgviewregistration.Rows(k).Cells(0)
