...
#End Region
#Region "Class frmMonthlyFees..."
Public Class frmMonthlyFees
#Region "Declarations..."
    ...
    Private MonthlyFee As Decimal = 0.0
...
#Region "Methods..."
    ...
    Public Function CodeIncrement(ByVal CODE As String) As String
        ...
        Temp += 1
        TT = String.Format("MNF:{0:00000000}", Temp)
        Return TT
    End Function
    ...
    Private Sub LoadMonthlyFees(ByVal StId As String)
        Try
            ...
            dgviewmonthly.Rows(0).Cells(1).Value = "Monthly Fees:"
            ...
            dgviewmonthly.Rows(6).Cells(0).Value = ISDMonthlyFeesAccount
    ...
    Private Sub LoadMonthlyFees(ByVal course As String, ByVal clas As String, ByVal roll As String)
        Try
            dgviewmonthly.Rows.Clear()
            dgviewmonthly.Rows.Add()
            dgviewmonthly.Rows.Add()
                ...
                If result = Windows.Forms.DialogResult.Yes Then
                    ...
                    If TokenExist = False Then
                        ...
                        Else
                            monrs = CDec(dgviewmonthly.Rows(0).Cells(2).Value.ToString())
                        End If
                        If CDec(dgviewmonthly.Rows(3).Cells(2).Value.ToString().Trim()) = 0.0 Then
                            otherrs = 0.0
                        Else
                            otherrs = CDec(dgviewmonthly.Rows(3).Cells(2).Value.ToString())
                        End If
                        ...
                        End If
                        If CDec(dgviewmonthly.Rows(6).Cells(2).Value.ToString()) = 0.0 Then
                            totamt = 0.0
                        Else
                        ...
                        If cls.AddMonthlyOk = True Then
                            ...
                            AddLibraryRs()
                            AddOtherRs()
                            cmd.CommandText = "update Accounts set AccountBalance=AccountBalance+" & CDec(dgviewmonthly.Rows(6).Cells(2).Value.ToString()) & " where InsID='" & My.Settings.LastOpenedInstituteID & "' and InsName='" & My.Settings.LastOpenedInstitute & "' and Period='" & My.Settings.Period & "' and AccountID='" & ISDMonthlyFeesAccount & "'"
                            dr = cmd.ExecuteReader
                            dr.Close()
    ...
    Private Sub AddOtherRs()
        Try
            ...
            Dim month As Integer = dt.Month
            Dim year As Integer = dt.Year
            cls.AddRecord(My.Settings.LastOpenedInstituteID, My.Settings.LastOpenedInstitute, My.Settings.Period, UmbrellaSystemDate, txttoken.Text, dt.ToShortDateString(), month, year, cmbcourse.Text, cmbclass.Text, cmbroll.Text, StuID, txtname.Text, txttime.Text, cmbpayvia.Text, reason, otherrs, discch, disc, totalamt, pay, remaining, paid)
            If cls.AddOtherFeesOk = False Then
                MsgBox("Cannot add other fees.")
            Else
                Dim description As String = "Other Fees"
                Dim type As String = "Income"
                cmd.CommandText = "insert into IncomeExpenditure(InsID,InsName,Period,SystemDate,transactionid,descriptions,type,amount,class,roll,regno,name,month,year,account) values ('" & My.Settings.LastOpenedInstituteID & "','" & My.Settings.LastOpenedInstitute & "','" & My.Settings.Period & "','" & UmbrellaSystemDate & "','" & txttoken.Text & "','" & description & "','" & type & "'," & otherrs & ",'" & cmbclass.Text & "','" & cmbroll.Text & "','" & StuID & "','" & txtname.Text & "'," & month & "," & year & ",'" & ISDOtherFeesAccount & "')"
                dr = cmd.ExecuteReader
                dr.Close()
                cmd.CommandText = "update Accounts set AccountBalance=AccountBalance+" & CDec(dgviewmonthly.Rows(3).Cells(2).Value.ToString()) & " where InsID='" & My.Settings.LastOpenedInstituteID & "' and InsName='" & My.Settings.LastOpenedInstitute & "' and Period='" & My.Settings.Period & "' and AccountID='" & ISDOtherFeesAccount & "'"
                dr = cmd.ExecuteReader
                dr.Close()
            End If
        Catch ex As Exception
        ...
        Try
            ...
            Else
                ...
                If result = Windows.Forms.DialogResult.Yes Then
                    ...
                    If cls.UpdateMonthlyOk = True Then
                        cmd.CommandText = "update Accounts set AccountBalance=AccountBalance-" & TotalPaid & " where InsID='" & My.Settings.LastOpenedInstituteID & "' and InsName='" & My.Settings.LastOpenedInstitute & "' and Period='" & My.Settings.Period & "' and AccountID='" & ISDMonthlyFeesAccount & "'"
                        dr = cmd.ExecuteReader
                        dr.Close()
                        ...
                        dr = cmd.ExecuteReader
                        dr.Close()
                        cmd.CommandText = "update Accounts set AccountBalance=AccountBalance+" & CDec(txtpaid.Text) & " where InsID='" & My.Settings.LastOpenedInstituteID & "' and InsName='" & My.Settings.LastOpenedInstitute & "' and Period='" & My.Settings.Period & "' and AccountID='" & ISDMonthlyFeesAccount & "'"
                        dr = cmd.ExecuteReader
                        dr.Close()
                        Dim description As String = "Monthly Fees"
                        Dim type As String = "Income"
                        cmd.CommandText = "update IncomeExpenditure set SystemDate='" & UmbrellaSystemDate & "',amount=" & paidamt & ",class='" & cmbclass.Text & "',roll='" & cmbroll.Text & "',regno='" & StuID & "',name='" & txtname.Text & "',month=" & month & ",year=" & year & ",account='" & dgviewmonthly.Rows(6).Cells(0).Value.ToString() & "' where InsID='" & My.Settings.LastOpenedInstituteID & "' and InsName='" & My.Settings.LastOpenedInstitute & "' and Period='" & My.Settings.Period & "' and transactionid='" & txttoken.Text & "' and descriptions='" & description & "' and type='" & type & "'"
                        dr = cmd.ExecuteReader
                        dr.Close()
                        UpdateDiscount()
                        UpdateLateRs()
        ...
        Try
            ...
            Else
                ...
                If result = Windows.Forms.DialogResult.Yes Then
                    ...
                    If cls.DeleteMonthlyOk = True Then
                        cmd.CommandText = "update Accounts set AccountBalance=AccountBalance-" & TotalPaid & " where InsID='" & My.Settings.LastOpenedInstituteID & "' and InsName='" & My.Settings.LastOpenedInstitute & "' and Period='" & My.Settings.Period & "' and AccountID='" & ISDMonthlyFeesAccount & "'"
                        dr = cmd.ExecuteReader
                        dr.Close()
                        cmd.CommandText = "update Accounts set AccountBalance=AccountBalance-" & LibraryFee & " where InsID='" & My.Settings.LastOpenedInstituteID & "' and InsName='" & My.Settings.LastOpenedInstitute & "' and Period='" & My.Settings.Period & "' and AccountID='" & ISDLibraryFeesAccount & "'"
    ...
    Private Sub LoadSelectedRecord()
        Try
            ...
            dgviewmonthly.Rows.Add()
            dgviewmonthly.Rows.Add()
            cmd.CommandText = "select * from MonthlyFees where InsID='" & My.Settings.LastOpenedInstituteID & "' and InsName='" & My.Settings.LastOpenedInstitute & "' and Period='" & My.Settings.Period & "' and regno='" & StuID & "' and date='" & monthlydate & "' and totalamt=" & monthlyamount & " and transactionid='" & transactionid & "'"
            dr = cmd.ExecuteReader
            If dr.Read() Then
                dt = dr("date").ToString()
                ...
                Dim monthly As Decimal = CDec(dr("monthlyrs").ToString())
                dgviewmonthly.Rows(0).Cells(0).Value = ""
                dgviewmonthly.Rows(0).Cells(1).Value = "Monthly Fees:"
                dgviewmonthly.Rows(0).Cells(2).Value = FormatNumber(monthly.ToString())
                MonthlyFee = monthly
                dgviewmonthly.Rows(1).Cells(0).Value = ISDLibraryFeesAccount
                ...
                LateFee = late
                dgviewmonthly.Rows(3).Cells(0).Value = ISDOtherFeesAccount
                Dim other As Decimal = CDec(dr("othersrs").ToString())
                dgviewmonthly.Rows(3).Cells(1).Value = "Other Fees:"
                ...
                txtremaining.Text = dr("remainingamt").ToString()
    ...
    Private Sub CheckTokenNo()
        Try
            cmd.CommandText = "select * from MonthlyFees where transactionid='" & txttoken.Text & "' and InsID='" & My.Settings.LastOpenedInstituteID & "' and InsName='" & My.Settings.LastOpenedInstitute & "' and Period='" & My.Settings.Period & "'"
            dr = cmd.ExecuteReader
            If dr.Read() Then
                TokenExist = True
            Else
...
#End Region
#Region "Form Events..."
    Private Sub frmMonthlyFees_Load(ByVal sender As Object, ByVal e As System.EventArgs) Handles Me.Load
        If updatemonthlyfees = False Then
            ControlSetBound()
            LoadCourse()
            If ISDAutoID = "Yes" Then
                GetCode()
    ...
    Private Sub cmbroll_KeyDown(ByVal sender As Object, ByVal e As System.Windows.Forms.KeyEventArgs) Handles cmbroll.KeyDown
        If e.KeyCode = Keys.Enter Then
            If cmbclass.Text <> "" And cmbcourse.Text <> "" And cmbroll.Text <> "" Then
                LoadMonthlyFees(cmbcourse.Text, cmbclass.Text, cmbroll.Text)
...
#Region "Button Events..."
    ...
    Private Sub btndelete_ClickButtonArea(ByVal Sender As System.Object, ByVal e As System.Windows.Forms.MouseEventArgs) Handles btndelete.ClickButtonArea
        If updatemonthlyfees = True Then
            DeleteRecord()
        End If
    End Sub
