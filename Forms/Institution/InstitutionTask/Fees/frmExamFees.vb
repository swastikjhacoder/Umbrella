...
#End Region
#Region "Class frmExamFees..."
Public Class frmExamFees
#Region "Declarations..."
    ...
    Private ExamFee As Decimal = 0.0
...
#Region "Methods..."
    ...
    Public Function CodeIncrement(ByVal CODE As String) As String
        ...
        Temp += 1
        TT = String.Format("XMF:{0:00000000}", Temp)
        Return TT
    End Function
    ...
    Private Sub ControlSetBound()
        ...
        Label10.SetBounds(185, 50, 17, 13)
        Label11.SetBounds(185, 77, 17, 13)
        Label12.SetBounds(10, 104, 55, 13)
        ...
        cmbexam.SetBounds(119, 20, 150, 21)
        cmbpayvia.SetBounds(119, 101, 60, 22)
        ...
        txtpaid.SetBounds(119, 47, 60, 21)
        txttime.SetBounds(61, 37, 379, 21)
        txttoken.SetBounds(79, 17, 136, 21)
        txtremaining.SetBounds(119, 74, 60, 21)
    End Sub
    ...
    Private Sub LoadExamFees(ByVal StId As String)
        Try
            ...
            dgviewmonthly.Rows(0).Cells(0).Value = ISDExamFeesAccount
            dgviewmonthly.Rows(0).Cells(1).Value = "Exam Fees:"
            ...
            dgviewmonthly.Rows(3).Cells(0).Value = ""
            dgviewmonthly.Rows(3).Cells(1).Value = "Discount Rate:"
            dgviewmonthly.Rows(3).Cells(2).Value = FormatNumber(disccharge.ToString())

            Dim latefees As Decimal = 0.0
            Dim xmrs As Decimal = 0.0
            ...
            If dgviewmonthly.Rows(0).Cells(2).Value.ToString().Trim() <> "" Or CDec(dgviewmonthly.Rows(0).Cells(2).Value.ToString()) <> 0.0 Then
                xmrs = CDec(dgviewmonthly.Rows(0).Cells(2).Value.ToString())
            Else
                xmrs = 0.0
            End If
            ...
            Dim totalamt As Decimal = xmrs + latefees + librs

            dgviewmonthly.Rows(4).Cells(0).Value = ISDDiscountFeesAccount
            dgviewmonthly.Rows(4).Cells(1).Value = "Discount:"
            If dgviewmonthly.Rows(3).Cells(2).Value.ToString().Trim() <> "" Or CDec(dgviewmonthly.Rows(3).Cells(2).Value.ToString()) <> 0.0 Then
                Dim disc As Decimal = (totalamt * CDec(dgviewmonthly.Rows(3).Cells(2).Value.ToString())) / 100
                dgviewmonthly.Rows(4).Cells(2).Value = Decimal.Round(disc, 2, MidpointRounding.AwayFromZero)
            End If

            dgviewmonthly.Rows(5).Cells(0).Value = ISDExamFeesAccount
            dgviewmonthly.Rows(5).Cells(1).Value = "Total:"
            If dgviewmonthly.Rows(4).Cells(2).Value.ToString().Trim() <> "" Or CDec(dgviewmonthly.Rows(4).Cells(2).Value.ToString()) <> 0.0 Then
                Dim totamt As Decimal = totalamt - CDec(dgviewmonthly.Rows(4).Cells(2).Value.ToString())
                dgviewmonthly.Rows(5).Cells(2).Value = Decimal.Round(totamt, 2, MidpointRounding.AwayFromZero)
            Else
                dgviewmonthly.Rows(5).Cells(2).Value = Decimal.Round(totalamt, 2, MidpointRounding.AwayFromZero)
            End If

            txtpaid.Text = dgviewmonthly.Rows(5).Cells(2).Value.ToString()
    ...
    Private Sub LoadExamFees(ByVal course As String, ByVal clas As String, ByVal roll As String)
        Try
            dgviewmonthly.Rows.Clear()
            dgviewmonthly.Rows.Add()
            dgviewmonthly.Rows.Add()
    ...
    Private Sub LoadExamInfo(ByVal course As String)
        Try
            ...
            cmbexam.AddDataGridView(DGVExam, False)
        Catch ex As Exception
    ...
    Private Sub CheckPaid()
        Try
            ...
            Dim month As Integer = dt.Month
            Dim year As Integer = dt.Year
            cmd.CommandText = "select paid from ExamFee where InsID='" & My.Settings.LastOpenedInstituteID & "' and InsName='" & My.Settings.LastOpenedInstitute & "' and Period='" & My.Settings.Period & "' and exam='" & cmbexam.Text & "' and month=" & month & " and year=" & year & " and course='" & cmbcourse.Text & "' and regno='" & StuID & "' and name='" & txtname.Text & "'"
            dr = cmd.ExecuteReader
            If dr.Read() Then
        ...
        Try
            ...
            ElseIf txtpaid.Text = "" Then
                ...
                If result = Windows.Forms.DialogResult.OK Then
                    txtpaid.Focus()
                End If
            ElseIf cmbexam.Text = "" Then
                Dim result As DialogResult = MessageBox.Show("Please enter/select exam info.", "Umbrella - Missing Value", MessageBoxButtons.OK, MessageBoxIcon.Warning)
                If result = Windows.Forms.DialogResult.OK Then
                    cmbexam.Focus()
                End If
            Else
                ...
                If result = Windows.Forms.DialogResult.Yes Then
                    ...
                    If TokenExist = False Then
                        Dim cls As New cExamFees()
                        Dim paid As String = ""
                        If CDec(dgviewmonthly.Rows(5).Cells(2).Value.ToString()) <= CDec(txtpaid.Text) Then
                            paid = "Paid"
                        Else
                            paid = "Unpaid"
                        End If
                        ...
                        If CDec(dgviewmonthly.Rows(0).Cells(2).Value.ToString()) = 0.0 Then
                            xmrs = 0.0
                        Else
                            xmrs = CDec(dgviewmonthly.Rows(0).Cells(2).Value.ToString())
                        End If
                        ...
                        End If
                        If CDec(dgviewmonthly.Rows(1).Cells(2).Value.ToString().Trim()) = 0.0 Then
                            librs = 0.0
                        Else
                            librs = CDec(dgviewmonthly.Rows(1).Cells(2).Value.ToString())
                        End If
                        If CDec(dgviewmonthly.Rows(3).Cells(2).Value.ToString()) = 0.0 Then
                            discch = 0.0
                        Else
                            discch = CDec(dgviewmonthly.Rows(3).Cells(2).Value.ToString())
                        End If
                        If CDec(dgviewmonthly.Rows(4).Cells(2).Value.ToString()) = 0.0 Then
                            disc = 0.0
                        Else
                            disc = CDec(dgviewmonthly.Rows(4).Cells(2).Value.ToString())
                        End If
                        If CDec(dgviewmonthly.Rows(5).Cells(2).Value.ToString()) = 0.0 Then
                            totamt = 0.0
                        Else
                            totamt = CDec(dgviewmonthly.Rows(5).Cells(2).Value.ToString())
                        End If
                        ...
                        Dim month As Integer = dt.Month
                        Dim year As Integer = dt.Year
                        cls.AddRecord(My.Settings.LastOpenedInstituteID, My.Settings.LastOpenedInstitute, My.Settings.Period, UmbrellaSystemDate, txttoken.Text, cmbdate.Text, month, year, cmbcourse.Text, cmbclass.Text, cmbroll.Text, StuID, txtname.Text, txttime.Text, cmbpayvia.Text, cmbexam.Text, xmrs, laters, librs, discch, disc, totamt, paidamt, remainingamt, paid)
                        If cls.AddExamFeeOk = True Then
                            Dim description As String = "Exam Fees"
                            Dim type As String = "Income"
                            cmd.CommandText = "insert into IncomeExpenditure(InsID,InsName,Period,SystemDate ,transactionid,descriptions,type,amount,class,roll,regno,name,month,year,account) values ('" & My.Settings.LastOpenedInstituteID & "','" & My.Settings.LastOpenedInstitute & "','" & My.Settings.Period & "','" & UmbrellaSystemDate & "','" & txttoken.Text & "','" & description & "','" & type & "'," & paidamt & ",'" & cmbclass.Text & "','" & cmbroll.Text & "','" & StuID & "','" & txtname.Text & "'," & month & "," & year & ",'" & ISDExamFeesAccount & "')"
                            dr = cmd.ExecuteReader
                            dr.Close()
                            AddDiscount()
                            AddLateRs()
                            AddLibraryRs()
                            cmd.CommandText = "update Accounts set AccountBalance=AccountBalance+" & CDec(txtpaid.Text) & " where InsID='" & My.Settings.LastOpenedInstituteID & "' and InsName='" & My.Settings.LastOpenedInstitute & "' and Period='" & My.Settings.Period & "' and AccountID='" & ISDExamFeesAccount & "'"
                            dr = cmd.ExecuteReader
                            dr.Close()
    ...
    Private Sub AddLateRs()
        Try
            ...
            If dgviewmonthly.Rows(5).Cells(2).Value.ToString().Trim() = "" Then
                totalamt = 0.0
            Else
                totalamt = CDec(dgviewmonthly.Rows(5).Cells(2).Value.ToString())
            End If
            If txtpaid.Text.Trim() = "" Then
                pay = 0.0
            Else
                pay = CDec(txtpaid.Text)
            End If
            ...
            Dim month As Integer = dt.Month
            Dim year As Integer = dt.Year
            cls.AddRecord(My.Settings.LastOpenedInstituteID, My.Settings.LastOpenedInstitute, My.Settings.Period, UmbrellaSystemDate, txttoken.Text, dt.ToShortDateString(), month, year, cmbcourse.Text, cmbclass.Text, cmbroll.Text, StuID, txtname.Text, txttime.Text, cmbpayvia.Text, reason, laters, discch, disc, totalamt, pay, remaining, paid)
            If cls.AddLateFeesOk = False Then
                MsgBox("Cannot add late fees.")
            Else
                Dim description As String = "Late Fees"
                Dim type As String = "Income"
                cmd.CommandText = "insert into IncomeExpenditure(InsID,InsName,Period,SystemDate,transactionid,descriptions,type,amount,class,roll,regno,name,month,year,account) values ('" & My.Settings.LastOpenedInstituteID & "','" & My.Settings.LastOpenedInstitute & "','" & My.Settings.Period & "','" & UmbrellaSystemDate & "','" & txttoken.Text & "','" & description & "','" & type & "'," & CDec(dgviewmonthly.Rows(2).Cells(2).Value.ToString()) & ",'" & cmbclass.Text & "','" & cmbroll.Text & "','" & StuID & "','" & txtname.Text & "'," & month & "," & year & ",'" & ISDLateFeesAccount & "')"
                dr = cmd.ExecuteReader
                dr.Close()
                cmd.CommandText = "update Accounts set AccountBalance=AccountBalance+" & CDec(dgviewmonthly.Rows(2).Cells(2).Value.ToString()) & " where InsID='" & My.Settings.LastOpenedInstituteID & "' and InsName='" & My.Settings.LastOpenedInstitute & "' and Period='" & My.Settings.Period & "' and AccountID='" & ISDLateFeesAccount & "'"
                dr = cmd.ExecuteReader
                dr.Close()
            End If
        Catch ex As Exception
    ...
    Private Sub AddLibraryRs()
        Try
            ...
            Dim month As Integer = dt.Month
            Dim year As Integer = dt.Year
            cls.AddRecord(My.Settings.LastOpenedInstituteID, My.Settings.LastOpenedInstitute, My.Settings.Period, UmbrellaSystemDate, txttoken.Text, dt.ToShortDateString(), month, year, cmbcourse.Text, cmbclass.Text, cmbroll.Text, StuID, txtname.Text, txttime.Text, cmbpayvia.Text, librs, laters, discch, disc, totalamt, pay, remaining, paid)
            If cls.AddLibraryFeeOk = False Then
                MsgBox("Cannot add library fees.")
            Else
                Dim description As String = "Library Fees"
                Dim type As String = "Income"
                cmd.CommandText = "insert into IncomeExpenditure(InsID,InsName,Period,SystemDate ,transactionid,descriptions,type,amount,class,roll,regno,name,month,year,account) values ('" & My.Settings.LastOpenedInstituteID & "','" & My.Settings.LastOpenedInstitute & "','" & My.Settings.Period & "','" & UmbrellaSystemDate & "','" & txttoken.Text & "','" & description & "','" & type & "'," & librs & ",'" & cmbclass.Text & "','" & cmbroll.Text & "','" & StuID & "','" & txtname.Text & "'," & month & "," & year & ",'" & ISDLibraryFeesAccount & "')"
                dr = cmd.ExecuteReader
                dr.Close()
                cmd.CommandText = "update Accounts set AccountBalance=AccountBalance+" & CDec(dgviewmonthly.Rows(1).Cells(2).Value.ToString()) & " where InsID='" & My.Settings.LastOpenedInstituteID & "' and InsName='" & My.Settings.LastOpenedInstitute & "' and Period='" & My.Settings.Period & "' and AccountID='" & ISDLibraryFeesAccount & "'"
                dr = cmd.ExecuteReader
                dr.Close()
            End If
        Catch ex As Exception
    ...
    Private Sub AddDiscount()
        Try
            ...
            Dim month As Integer = dt.Month
            Dim year As Integer = dt.Year
            cls.AddRecord(My.Settings.LastOpenedInstituteID, My.Settings.LastOpenedInstitute, My.Settings.Period, UmbrellaSystemDate, txttoken.Text, dt.ToShortDateString(), month, year, appliedfor, cmbclass.Text, cmbroll.Text, StuID, txtname.Text, totalamt, discch, disc, disc, cmbpayvia.Text, authentication, remarks, ISDDiscountFeesAccount)
            If cls.AddDiscountOk = False Then
                MsgBox("Cannot add discount")
            Else
                cmd.CommandText = "update Accounts set AccountBalance=AccountBalance-" & CDec(dgviewmonthly.Rows(4).Cells(2).Value.ToString()) & " where InsID='" & My.Settings.LastOpenedInstituteID & "' and InsName='" & My.Settings.LastOpenedInstitute & "' and Period='" & My.Settings.Period & "' and AccountID='" & ISDDiscountFeesAccount & "'"
                dr = cmd.ExecuteReader
                dr.Close()
            End If
        Catch ex As Exception
        ...
        Try
            ...
            Else
                ...
                If result = Windows.Forms.DialogResult.Yes Then
                    ...
                    Dim month As Integer = dt.Month
                    Dim year As Integer = dt.Year
                    cls.UpdateRecord(My.Settings.LastOpenedInstituteID, My.Settings.LastOpenedInstitute, My.Settings.Period, UmbrellaSystemDate, txttoken.Text, cmbdate.Text, month, year, cmbcourse.Text, cmbclass.Text, cmbroll.Text, StuID, txtname.Text, txttime.Text, cmbpayvia.Text, cmbexam.Text, monrs, laters, librs, discch, disc, totamt, paidamt, remainingamt, paid)
                    If cls.UpdateExamFeeOk = True Then
                        cmd.CommandText = "update Accounts set AccountBalance=AccountBalance-" & TotalPaid & " where InsID='" & My.Settings.LastOpenedInstituteID & "' and InsName='" & My.Settings.LastOpenedInstitute & "' and Period='" & My.Settings.Period & "' and AccountID='" & ISDExamFeesAccount & "'"
                        dr = cmd.ExecuteReader
                        dr.Close()
                        ...
                        Dim description As String = "Exam Fees"
                        Dim type As String = "Income"
                        cmd.CommandText = "update IncomeExpenditure set SystemDate='" & UmbrellaSystemDate & "',amount=" & paidamt & ",class='" & cmbclass.Text & "',roll='" & cmbroll.Text & "',regno='" & StuID & "',name='" & txtname.Text & "',month=" & month & ",year=" & year & ",account='" & ISDExamFeesAccount & "' where InsID='" & My.Settings.LastOpenedInstituteID & "' and InsName='" & My.Settings.LastOpenedInstitute & "' and Period='" & My.Settings.Period & "' and transactionid='" & txttoken.Text & "' and descriptions='" & description & "' and type='" & type & "'"
                        dr = cmd.ExecuteReader
                        dr.Close()
                        UpdateDiscount()
                        UpdateLateRs()
    ...
    Private Sub UpdateDiscount()
        Try
            ...
            Dim month As Integer = dt.Month
            Dim year As Integer = dt.Year
            cls.UpdateRecord(My.Settings.LastOpenedInstituteID, My.Settings.LastOpenedInstitute, My.Settings.Period, UmbrellaSystemDate, txttoken.Text, cmbdate.Text, month, year, appliedfor, cmbclass.Text, cmbroll.Text, StuID, txtname.Text, totalamt, discch, disc, CDec(dgviewmonthly.Rows(5).Cells(2).Value.ToString()), cmbpayvia.Text, authentication, remarks, ISDDiscountFeesAccount)
        ...
        Try
            ...
            Else
                ...
                If result = Windows.Forms.DialogResult.Yes Then
                    ...
                    If cls.DeleteExamFeeOk = True Then
                        cmd.CommandText = "update Accounts set AccountBalance=AccountBalance-" & TotalPaid & " where InsID='" & My.Settings.LastOpenedInstituteID & "' and InsName='" & My.Settings.LastOpenedInstitute & "' and Period='" & My.Settings.Period & "' and AccountID='" & ISDExamFeesAccount & "'"
                        dr = cmd.ExecuteReader
                        dr.Close()
                        cmd.CommandText = "update Accounts set AccountBalance=AccountBalance-" & LibraryFee & " where InsID='" & My.Settings.LastOpenedInstituteID & "' and InsName='" & My.Settings.LastOpenedInstitute & "' and Period='" & My.Settings.Period & "' and AccountID='" & ISDLibraryFeesAccount & "'"
    ...
    Private Sub LoadSelectedRecord()
        Try
            ...
            dgviewmonthly.Rows.Add()
            dgviewmonthly.Rows.Add()
            cmd.CommandText = "select * from ExamFee where InsID='" & My.Settings.LastOpenedInstituteID & "' and InsName='" & My.Settings.LastOpenedInstitute & "' and Period='" & My.Settings.Period & "' and regno='" & StuID & "' and date='" & examfeesdate & "' and totalamt=" & examfeesamount & " and transactionid='" & transactionid & "'"
            dr = cmd.ExecuteReader
            If dr.Read() Then
                dt = dr("date").ToString()
                ...
                cmbexam.Text = dr("exam").ToString()
                Dim xmrs As Decimal = CDec(dr("examfee").ToString())
                dgviewmonthly.Rows(0).Cells(0).Value = ISDExamFeesAccount
                dgviewmonthly.Rows(0).Cells(1).Value = "Exam Fees:"
                dgviewmonthly.Rows(0).Cells(2).Value = FormatNumber(xmrs.ToString())
                ExamFee = xmrs
                dgviewmonthly.Rows(1).Cells(0).Value = ISDLibraryFeesAccount
                ...
                dgviewmonthly.Rows(3).Cells(0).Value = ""
                dgviewmonthly.Rows(3).Cells(1).Value = "Discount Rate:"
                dgviewmonthly.Rows(3).Cells(2).Value = FormatNumber(discch.ToString())
                dgviewmonthly.Rows(4).Cells(0).Value = ISDDiscountFeesAccount
                Dim disc As Decimal = CDec(dr("discount").ToString())
                dgviewmonthly.Rows(4).Cells(1).Value = "Discount:"
                dgviewmonthly.Rows(4).Cells(2).Value = FormatNumber(disc.ToString())
                Discount = disc
                dgviewmonthly.Rows(5).Cells(0).Value = ISDExamFeesAccount
                Dim amt As Decimal = CDec(dr("totalamt").ToString())
                dgviewmonthly.Rows(5).Cells(1).Value = "Total:"
                dgviewmonthly.Rows(5).Cells(2).Value = FormatNumber(amt.ToString())
    ...
    Private Sub CheckTokenNo()
        Try
            cmd.CommandText = "select * from ExamFee where transactionid='" & txttoken.Text & "' and InsID='" & My.Settings.LastOpenedInstituteID & "' and InsName='" & My.Settings.LastOpenedInstitute & "' and Period='" & My.Settings.Period & "'"
            dr = cmd.ExecuteReader
            If dr.Read() Then
                TokenExist = True
            Else
...
#Region "Attributes..."
    ...
    Private Sub cmbroll_KeyDown(ByVal sender As Object, ByVal e As System.Windows.Forms.KeyEventArgs) Handles cmbroll.KeyDown
        If e.KeyCode = Keys.Enter Then
            If cmbclass.Text <> "" And cmbcourse.Text <> "" And cmbroll.Text <> "" Then
                LoadExamFees(cmbcourse.Text, cmbclass.Text, cmbroll.Text)
    ...
    Private Sub cmbexam_KeyDown(ByVal sender As Object, ByVal e As System.Windows.Forms.KeyEventArgs) Handles cmbexam.KeyDown
        If e.KeyCode = Keys.Enter Then
            txtpaid.Focus()
        End If
    End Sub
...
#Region "Button Events..."
    ...
    Private Sub btnremaining_ClickButtonArea(ByVal Sender As Object, ByVal e As System.Windows.Forms.MouseEventArgs) Handles btnremaining.ClickButtonArea
        Try
            Dim total As Decimal = CDec(dgviewmonthly.Rows(5).Cells(2).Value.ToString())
    ...
    Private Sub dgviewmonthly_CellEndEdit(ByVal sender As Object, ByVal e As System.Windows.Forms.DataGridViewCellEventArgs) Handles dgviewmonthly.CellEndEdit
        If e.ColumnIndex = 2 Then
            ...
            If dgviewmonthly.Rows(i).Cells(2).Value.ToString() = "" Then
                ...
                Dim discch As Decimal = CDec(dgviewmonthly.Rows(3).Cells(2).Value.ToString())
                Dim total As Decimal = monthly + library + late
                Dim disc As Decimal = (total * discch) / 100
                dgviewmonthly.Rows(4).Cells(2).Value = FormatNumber(disc.ToString())
                total = total - disc
                dgviewmonthly.Rows(5).Cells(2).Value = FormatNumber(total.ToString())
...
#End Region
#Region "Form Evenrts..."
    Private Sub frmExamFees_Load(ByVal sender As Object, ByVal e As System.EventArgs) Handles Me.Load
        If updateexamfees = False Then
            ControlSetBound()
            LoadCourse()
            If ISDAutoID = "Yes" Then
