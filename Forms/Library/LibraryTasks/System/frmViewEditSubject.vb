#Region "Class frmViewEditSubject..."
Public Class frmViewEditSubject
#Region "Methods..."
    Private Sub LoadDataGrid()
        Try
            Dim i As Integer = 0
            dgviewsubjects.Rows.Clear()
            cmd.CommandText = "select subjectid,subjectname,subjectdetails from SubjectBooks where InsID='" & My.Settings.LastOpenedInstituteID & "' and InsName='" & My.Settings.LastOpenedInstitute & "' and Period='" & My.Settings.Period & "' and SystemDate<='" & UmbrellaSystemDate & "'"
            dr = cmd.ExecuteReader
            While dr.Read()
                dgviewsubjects.Rows.Add()
                dgviewsubjects.Rows(i).Cells(0).Value = dr("subjectid").ToString()
                dgviewsubjects.Rows(i).Cells(1).Value = dr("subjectname").ToString()
                dgviewsubjects.Rows(i).Cells(2).Value = dr("subjectdetails").ToString()
                i = i + 1
            End While
            dr.Close()
    ...
    Private Sub OpenRecords()
        Try
            updatesubjectbooks = True
            Dim k As Integer = dgviewsubjects.CurrentRow.Index
            SubjectID = dgviewsubjects.Rows(k).Cells(0).Value.ToString()
    ...
    Private Sub SearchRecord()
        Try
            If txtsearch.Text <> "" Then
                If cmbsearch.Text = "Subject ID" Then
                    Dim i As Integer
                    dgviewsubjects.ClearSelection()
                    For i = 0 To dgviewsubjects.Rows.Count - 1
                        ...
                        End If
                    Next
                ElseIf cmbsearch.Text = "Subject Name" Then
                    Dim i As Integer
                    dgviewsubjects.ClearSelection()
                    For i = 0 To dgviewsubjects.Rows.Count - 1
                        ...
                        End If
                    Next
                ElseIf cmbsearch.Text = "Subject Details" Then
                    Dim i As Integer
                    dgviewsubjects.ClearSelection()
                    For i = 0 To dgviewsubjects.Rows.Count - 1
...
#End Region
#Region "Form Events..."
    Private Sub frmViewEditSubject_Load(ByVal sender As Object, ByVal e As System.EventArgs) Handles Me.Load
        LoadDataGrid()
        TotalSubjects()
    End Sub
#End Region
